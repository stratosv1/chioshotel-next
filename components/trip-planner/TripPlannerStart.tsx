...
type MarineResponse = {
  ok: boolean;
  generatedAt?: string;
  requestedWindow?: { startHourLocal: string; endHourLocal: string };
...
const ratingLabel = (rating: MarineRankedBeach["rating"]) => ({
  ...
  fair: "Μέτρια",
  poor: "Δύσκολη",
  "not-recommended": "Όχι καλή σήμερα",
}[rating]);
...
function SelectionStrip({
  ...
  selectedCategories.forEach((category) => {
    ...
    if (names.length) {
      names.forEach((name, index) => chips.push({ key: `${category}-${index}-${name}`, label: `${emojis[category]} ${name}` }));
    } else {
      chips.push({ key: `pending-${category}`, label: `${emojis[category]} ${categoryById[category].label}` });
    }
  });
    ...
    <div className="sticky top-0 z-30 border-b border-[#e8dfd4] bg-[#fffdf9]/95 backdrop-blur">
      <div className="mx-auto flex w-[min(1260px,calc(100%_-_24px))] items-center gap-2 py-2 md:w-[min(1260px,calc(100%_-_48px))]">
        ...
        <div className="flex min-w-0 flex-1 gap-1.5 overflow-x-auto py-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:flex-wrap md:overflow-visible">
          {chips.map((chip) => (
            <span key={chip.key} className="shrink-0 rounded-full border border-[#e3d8ca] bg-white px-2.5 py-1 text-[10px] font-semibold text-[#665548] shadow-sm md:text-[11px]">{chip.label}</span>
          ))}
        </div>
      </div>
    </div>
...
export default function TripPlannerStart() {
    ...
    if (currentCategory === "beach") {
      return beachItems
        ...
        .map((item) => ({
          ...
          name: item.name,
          image: item.image,
          meta: item.bestTime ?? item.character?.[0] ?? "Παραλία Χίου",
    ...
    if (currentCategory === "village") {
      return villageItems
        ...
        .map((item) => ({
          ...
          name: item.name,
          image: item.image,
          meta: `${item.bestTime} · ${item.recommendedDuration}`,
  ...
  const toggleCategory = (id: CategoryId) => {
    ...
    if (selected.includes(id)) {
      setChosenPlaces((current) => ({ ...current, [id]: [] }));
    }
  };
  ...
  const togglePlace = (id: string) => {
    ...
    setChosenPlaces((current) => {
      ...
      if (!MULTI_SELECT.has(currentCategory)) {
        return { ...current, [currentCategory]: existing[0] === id ? [] : [id] };
      }
      return {
        ...current,
        [currentCategory]: existing.includes(id)
            ...
            {regionOptions.map((item) => {
              ...
              return (
                <button key={item.id} type="button" onClick={() => chooseRegion(item.id)} className={`${item.position} h-full w-full rounded-[18px] border px-2 text-center shadow-[0_7px_18px_rgba(65,48,36,.055)] transition md:rounded-[20px] ${best ? "border-[#aeb39a] bg-[#eef0e6] text-[#5f684d] ring-2 ring-[#aeb39a]/20" : "border-[#e5ddd3] bg-white text-[#4f463f] hover:border-[#aeb39a] hover:bg-[#f6f7f1]"}`}>
                  ...
                  <span className="mt-0.5 block text-[9px] font-semibold opacity-75 md:text-[10px]">{best ? "★ Καλύτερα σήμερα" : info?.score !== null && info?.score !== undefined ? `${info.score}/100` : item.title}</span>
                </button>
              );
            })}
      ...
      {step === "places" && currentCategory && currentCategoryInfo ? (
        <section className="mx-auto w-[min(1060px,calc(100%_-_24px))] py-4 pb-24 md:py-7 md:pb-10">
          ...
          <div className="text-center">
            ...
            {currentCategory === "beach" ? <p className="mx-auto mt-1 max-w-[680px] text-[10px] leading-4 text-[#9a8877]">Οι παραλίες ταξινομούνται με βάση τη σημερινή εκτίμηση θάλασσας. Τα στοιχεία είναι πρόγνωση και όχι live μέτρηση ή εγγύηση ασφάλειας.</p> : null}
          </div>
          ...
          <div className={`mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 md:mt-6 ${currentCategory === "beach" ? "lg:grid-cols-3" : "md:grid-cols-3"}`}>
              ...
              return (
                <button key={item.id} type="button" aria-pressed={isSelected} onClick={() => togglePlace(item.id)} className={`overflow-hidden rounded-[16px] border bg-white text-left shadow-[0_7px_20px_rgba(65,48,36,.06)] transition hover:-translate-y-0.5 ${isSelected ? "border-[#aeb39a] bg-[#fdfdf9] ring-2 ring-[#aeb39a]/30" : "border-[#e3dacf] hover:border-[#c9b9a7]"}`}>
                  ...
                  <div className="p-3 md:p-4">
                    <div className="font-serif text-[19px] font-semibold text-[#332923] md:text-[21px]">{item.name}</div>
          ...
          {MULTI_SELECT.has(currentCategory) && currentChosenCount >= 4 ? <div className="mx-auto mt-4 max-w-[620px] rounded-xl border border-[#ead7bd] bg-[#fff7e8] px-4 py-3 text-center text-[11px] leading-4 text-[#7b623f]">Έχεις επιλέξει {currentChosenCount} {currentCategoryInfo.plural}. Είναι αρκετές στάσεις για μία μέρα· αργότερα ο planner θα τις οργανώσει με βάση χρόνο και αποστάσεις.</div> : null}

          <div className="sticky bottom-3 z-20 mx-auto mt-5 w-full rounded-2xl border border-[#ded7ca] bg-[#fffdf9]/95 p-2 shadow-[0_14px_34px_rgba(56,43,33,.16)] backdrop-blur md:static md:w-[420px] md:border-0 md:bg-transparent md:p-0 md:shadow-none">
