...
const beachLoversStayImage =
  "relative overflow-hidden rounded-[32px] bg-slate-200 shadow-[0_20px_44px_rgba(15,23,42,0.14)]";
...
export function BeachLoversPage({ data }: BeachLoversPageProps) {
  ...
  return (
    <main className="bg-[#fffaf2] text-slate-900">
      <section className={beachLoversHero}>
        <div className={beachLoversHeroImage}>
          <Image
            src={data.hero.image}
            alt={data.hero.title}
            fill
          ...
          />
          <div className={beachLoversHeroOverlay} />
        </div>
        ...
        <div className={beachLoversSection}>
          ...
          <div className={beachLoversGrid}>
            {data.beaches.map((beach, index) => (
              <Link key={beach.name} href={beach.href} className={beachLoversCard}>
