...
export default function TasteLoverPage({ data }: TasteLoverPageProps) {
  return (
    <main className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top_left,rgba(168,120,66,.14),transparent_34rem),linear-gradient(180deg,#fffaf4_0%,#f4eadf_55%,#fffaf4_100%)] text-stone-800">
      <section className="relative flex min-h-[640px] items-end overflow-hidden text-white max-md:min-h-[76svh]" aria-labelledby="taste-lover-title">
        ...
        <div className="relative z-[2] mx-auto w-[min(1180px,calc(100%-40px))] py-20 pt-28 max-md:w-[calc(100%-24px)] max-md:py-14 max-md:pt-6">
          <div className="w-[min(780px,100%)] rounded-[2.125rem] border border-white/25 bg-white/10 p-[clamp(30px,5vw,56px)] shadow-[0_34px_90px_rgba(0,0,0,.28)] backdrop-blur-xl max-md:border-0 max-md:bg-transparent max-md:p-0 max-md:shadow-none max-md:backdrop-blur-0">
            ...
            <h1 id="taste-lover-title" className="m-0 max-w-[12ch] text-[clamp(42px,7vw,86px)] font-black leading-[0.94] tracking-[-0.06em] text-white drop-shadow-lg">{data.hero.title}</h1>
      ...
      <section className="py-16 md:py-20" aria-labelledby="taste-intro-title">
        <div className="mx-auto grid w-[min(1180px,calc(100%-40px))] items-start gap-8 max-md:w-[calc(100%-24px)] lg:grid-cols-[minmax(0,.85fr)_minmax(0,1.15fr)]">
          <div>
            <p className="mb-4 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-amber-900 before:h-px before:w-8 before:bg-current before:opacity-75">{data.hero.eyebrow}</p>
            <h2 id="taste-intro-title" className="m-0 text-[clamp(32px,4.6vw,58px)] font-black leading-none tracking-[-0.055em] text-stone-800">{data.intro.title}</h2>
          </div>
          <p className="text-base leading-8 text-stone-600 md:text-lg">{data.intro.text}</p>
        </div>
      </section>

      <section className="bg-white/70 py-16 md:py-20" aria-labelledby="taste-highlights-title">
        <div className="mx-auto w-[min(1180px,calc(100%-40px))] max-md:w-[calc(100%-24px)]">
          <header className="mx-auto mb-9 max-w-[820px] text-center">
            <p className="mb-4 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-amber-900 before:h-px before:w-8 before:bg-current before:opacity-75">{data.hero.eyebrow}</p>
            <h2 id="taste-highlights-title" className="m-0 text-[clamp(32px,4.6vw,58px)] font-black leading-none tracking-[-0.055em] text-stone-800">{data.highlights.title}</h2>
            <p className="mx-auto mt-5 max-w-[760px] text-base leading-8 text-stone-600">{data.highlights.subtitle}</p>
          </header>
      ...
      <section className="py-16 md:py-20" aria-labelledby="taste-experience-title">
        <div className="mx-auto w-[min(1180px,calc(100%-40px))] max-md:w-[calc(100%-24px)]">
          <header className="mx-auto mb-9 max-w-[820px] text-center">
            <p className="mb-4 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-amber-900 before:h-px before:w-8 before:bg-current before:opacity-75">{data.hero.eyebrow}</p>
            <h2 id="taste-experience-title" className="m-0 text-[clamp(32px,4.6vw,58px)] font-black leading-none tracking-[-0.055em] text-stone-800">{data.experiences.title}</h2>
            <p className="mx-auto mt-5 max-w-[760px] text-base leading-8 text-stone-600">{data.experiences.subtitle}</p>
          </header>
      ...
      <section className="bg-white/70 py-16 md:py-20" aria-labelledby="taste-day-title">
        <div className="mx-auto w-[min(1180px,calc(100%-40px))] max-md:w-[calc(100%-24px)]">
          <header className="mx-auto mb-9 max-w-[820px] text-center">
            <p className="mb-4 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-amber-900 before:h-px before:w-8 before:bg-current before:opacity-75">{data.hero.eyebrow}</p>
            <h2 id="taste-day-title" className="m-0 text-[clamp(32px,4.6vw,58px)] font-black leading-none tracking-[-0.055em] text-stone-800">{data.day.title}</h2>
            <p className="mx-auto mt-5 max-w-[760px] text-base leading-8 text-stone-600">{data.day.subtitle}</p>
          </header>

          <div className="grid gap-4">
            {data.day.steps.map((step) => (
              <article className="grid gap-4 rounded-[1.5rem] border border-amber-900/10 bg-white p-5 shadow-lg shadow-stone-900/5 md:grid-cols-[140px_1fr]" key={`${step.time}-${step.title}`}>
                <div className="rounded-full bg-[#fff4df] px-4 py-3 text-center text-sm font-black uppercase tracking-[0.1em] text-amber-900 ring-1 ring-amber-900/10">{step.time}</div>
                <div>
      ...
      <section className="py-16 md:py-20" aria-labelledby="taste-stay-title">
        <div className="mx-auto grid w-[min(1180px,calc(100%-40px))] items-start gap-7 max-md:w-[calc(100%-24px)] lg:grid-cols-[minmax(0,1fr)_minmax(320px,.8fr)]">
          <article className="rounded-[2rem] border border-amber-900/10 bg-white/90 p-[clamp(30px,5vw,48px)] shadow-xl shadow-stone-900/5">
            <p className="mb-4 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.16em] text-amber-900 before:h-px before:w-8 before:bg-current before:opacity-75">Voulamandis House</p>
            <h2 id="taste-stay-title" className="m-0 text-[clamp(32px,4.6vw,58px)] font-black leading-none tracking-[-0.055em] text-stone-800">{data.stay.title}</h2>
          ...
          <aside className="rounded-[2rem] bg-gradient-to-br from-stone-900 to-stone-700 p-[clamp(28px,4vw,44px)] text-white shadow-2xl shadow-stone-900/15">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Link className={darkButton} href={data.links.rooms}>{data.finalCta.secondaryCta}</Link>
            </div>
          </aside>
        </div>
      </section>
