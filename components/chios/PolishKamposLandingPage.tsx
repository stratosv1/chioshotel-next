...
const rooms = [
  {
    ...
    href: "/pl/pokoje-na-chios/economy-double/",
  },
  {
    ...
    href: "/pl/pokoje-na-chios/standard/",
  },
  {
    ...
    <main className="overflow-hidden bg-[#f8f4ee] text-[#493a31]">
        ...
        <div className={`${shell} grid items-center gap-6 pb-8 sm:gap-10 sm:pb-14 lg:grid-cols-[1.02fr_0.98fr] lg:gap-14`}>
          <div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2.5 sm:mt-8 sm:flex">
              <Cta href="/pl/pokoje-na-chios/">Zobacz pokoje</Cta>
              <Cta href="/pl/rezerwacja/" secondary>Sprawdź ceny</Cta>
            </div>
            <ul className="mt-5 grid grid-cols-2 gap-2 sm:mt-7 sm:grid-cols-4">
              {facts.map(([value, label]) => <li key={label} className="rounded-2xl border border-[#e4d7cb] bg-[#fffdf9]/90 px-3 py-3 shadow-sm"><strong className="block font-serif text-lg text-[#59473b] sm:text-xl">{value}</strong><span className="block text-[0.7rem] font-semibold text-[#8a7869] sm:text-xs">{label}</span></li>)}
            </ul>
          </div>
          <div className="relative aspect-[16/10] overflow-hidden rounded-[1.6rem] shadow-xl shadow-[#6f5949]/12 sm:aspect-[5/4] sm:rounded-[2.5rem] lg:aspect-[4/5]">
            <Image src="/images/kampos/kambos-chios.jpg" alt="Historyczny Kambos na Chios z kamiennymi rezydencjami i ogrodami cytrusowymi" fill priority className="object-cover" sizes="(max-width: 1024px) 100vw, 46vw" />
          </div>
        </div>
      </section>
      ...
      <section className={`${shell} py-9 sm:py-16`}>
        <div className="grid items-center gap-5 lg:grid-cols-[0.9fr_1.1fr] lg:gap-14">
          <div><p className={eyebrow}>Inny rytm pobytu</p><h2 className={`${heading} mt-3`}>Spokój, charakter i wygodna lokalizacja</h2></div>
          <div className="space-y-4 rounded-3xl border border-[#eadfd4] bg-[#fffdf9] p-5 shadow-sm sm:p-7">
            <p className={body}>Kambos to jedna z najbardziej charakterystycznych i historycznych części Chios, znana z kamiennych rezydencji, wysokich murów, ogrodów cytrusowych i spokojnych uliczek.</p>
            <p className={body}>Voulamandis House znajduje się właśnie tutaj. Dzięki położeniu blisko miasta i lotniska Kambos jest praktyczną bazą dla gości, którzy chcą zwiedzać wyspę, ale wieczorem wracać do spokojniejszego otoczenia.</p>
          </div>
        </div>
      </section>
      ...
      <section className={`${shell} py-9 sm:py-16`}>
        <div className="space-y-8 sm:space-y-12">
          ...
          ].map(([title, text, image], index) => <article key={title} className="grid items-center gap-6 lg:grid-cols-2 lg:gap-14"><div className={`relative aspect-[16/10] overflow-hidden rounded-[1.6rem] shadow-lg shadow-[#6f5949]/10 sm:aspect-[4/3] ${index % 2 ? "lg:order-2" : ""}`}><Image src={image} alt={title} fill className="object-cover" sizes="(max-width: 1024px) 100vw, 48vw" /></div><div><span className={eyebrow}>0{index + 1}</span><h2 className={`${heading} mt-3`}>{title}</h2><p className={`${body} mt-4`}>{text}</p></div></article>)}
        </div>
      </section>
      ...
      ].map(([question, answer]) => <details key={question} className="group rounded-2xl border border-[#e4d7cb] bg-[#f8f4ee] px-4 py-1"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 py-4 font-extrabold text-[#493a31]"><span>{question}</span><span aria-hidden="true">+</span></summary><p className="border-t border-[#e4d7cb] pb-4 pt-3 text-sm leading-7 text-[#75665b]">{answer}</p></details>)}</div></div></section>

      <section className={`${shell} pb-9 sm:pb-16`}><div className="relative overflow-hidden rounded-[1.6rem] bg-[#493a31] px-5 py-8 text-white sm:rounded-[2.5rem] sm:px-10 sm:py-14"><Image src="/images/kampos/kampos-chios-citrus-estate.webp" alt="" fill className="object-cover opacity-25" sizes="100vw" aria-hidden="true" /><div className="absolute inset-0 bg-[#493a31]/70" /><div className="relative z-10 max-w-3xl"><p className="text-[0.68rem] font-extrabold uppercase tracking-[0.22em] text-[#eadfd4]">Voulamandis House · Kambos Chios</p><h2 className="mt-3 font-serif text-[2rem] font-semibold leading-[1.02] text-white sm:text-4xl">Zamieszkaj pośród ogrodów cytrusowych</h2><p className="mt-3 text-sm leading-7 text-white/80 sm:text-lg">Wybierz pokój lub apartament i poznawaj Chios z cichej, historycznej części wyspy.</p><div className="mt-5 grid grid-cols-2 gap-2.5 sm:flex"><a href="/pl/rezerwacja/" className="inline-flex min-h-11 items-center justify-center rounded-full bg-white px-4 py-2.5 text-center text-[0.78rem] font-extrabold !text-[#493a31]">Ceny i dostępność</a><a href="/pl/pokoje-na-chios/" className="inline-flex min-h-11 items-center justify-center rounded-full border border-white/35 bg-white/10 px-4 py-2.5 text-center text-[0.78rem] font-extrabold !text-white">Zobacz pokoje</a></div></div></div></section>
    </main>
  );
}
