...
export function AIRoomFinder() {
  ...
  const whatsappHref = useMemo(() => {
    if (!activeOffer) return "#";
    const text = [t.whatsappIntro, "", `${activeOffer.name} · ${activeOffer.category}`, search.checkin ? `Check-in: ${search.checkin}` : "", search.checkout ? `Check-out: ${search.checkout}` : "", search.guests ? `${language === "el" ? "Επισκέπτες" : "Guests"}: ${search.guests}` : "", `${language === "el" ? "Τιμή" : "Price"}: ${money(activeOffer.directTotal)}`, "", language === "el" ? "Παρακαλώ επικοινωνήστε μαζί μου. Δεν πρόκειται για επιβεβαιωμένη κράτηση." : "Please contact me. This is not a confirmed booking."].filter(Boolean).join("\n");
  ...
  const manualWhatsappHref = useMemo(() => {
    const text = [t.manualWhatsapp, "", `${t.name}: ${manualName}`, `${t.contact}: ${manualContact}`, search.checkin ? `Check-in: ${search.checkin}` : "", search.checkout ? `Check-out: ${search.checkout}` : "", search.guests ? `${language === "el" ? "Επισκέπτες" : "Guests"}: ${search.guests}` : "", "", language === "el" ? "Με βάση τα πρόσφατα στοιχεία δεν προέκυψε επιβεβαιωμένη διαθεσιμότητα. Παρακαλώ ελέγξτε το άμεσα." : "No confirmed availability appeared in the latest data. Please check immediately."].filter(Boolean).join("\n");
    return `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(text)}`;
  ...
  return (
    <main className="min-h-[100dvh] bg-[#fbfaf7] text-stone-950">
      ...
      {activeOffer ? <div className="fixed inset-0 z-40 flex items-end justify-center bg-stone-950/60 sm:items-center sm:p-5" role="dialog" aria-modal="true"><div className="relative flex max-h-[96dvh] w-full max-w-3xl flex-col overflow-hidden rounded-t-[2rem] bg-white shadow-2xl sm:rounded-[2rem]"><div className="relative"><RoomCarousel images={gallery} roomName={activeOffer.name} /><button onClick={() => setActiveOffer(null)} className="absolute right-4 top-4 z-10 grid h-11 w-11 place-items-center rounded-full bg-white text-xl shadow" aria-label={t.close}>✕</button></div><div className="overflow-y-auto p-5 pb-28"><h2 className="text-2xl font-bold">{activeOffer.name}</h2><p className="mt-1 text-sm font-semibold text-[#b45309]">{activeOffer.category} · {t.upTo} {activeOffer.maxGuests} {t.guests}</p><div className="mt-4 flex flex-wrap gap-2">{[activeOffer.floor, ...activeOffer.features].filter(Boolean).map((feature) => <span key={feature} className="rounded-full border bg-stone-50 px-3 py-1.5 text-xs">{feature}</span>)}</div></div><div className="absolute inset-x-0 bottom-0 border-t bg-white p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs text-stone-400 line-through">{money(activeOffer.originalTotal)}</p><p className="text-2xl font-black text-[#43551b]">{money(activeOffer.directTotal)}</p></div><div className="grid grid-cols-2 gap-2"><button onClick={() => { setRequestOffer(activeOffer); setActiveOffer(null); setSuccess(""); }} className="rounded-2xl bg-stone-950 px-4 py-3 text-sm font-semibold text-white">✉️ {t.email}</button><a href={whatsappHref} target="_blank" rel="noopener noreferrer" className="rounded-2xl bg-[#25D366] px-4 py-3 text-center text-sm font-semibold text-white">💬 {t.whatsapp}</a></div></div></div></div></div> : null}
