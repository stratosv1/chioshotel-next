...
function roomNumber(value:string){ return Number(value.match(/(?:Room|Δωμάτιο|Zimmer|Chambre|Camera|Habitación|Oda|Apartment|Appartement|Appartamento|Apartamento|Daire|Διαμέρισμα)\s*(10|[1-9])/i)?.[1]||0); }
...
function readRoomCard(button:HTMLButtonElement,language:Language):RoomDetails|null{
  ...
  const numbers=Array.from(name.matchAll(/(?:Room|Δωμάτιο|Zimmer|Chambre|Camera|Habitación|Oda|Apartment|Appartement|Appartamento|Apartamento|Daire|Διαμέρισμα)\s*(10|[1-9])/gi)).map((m)=>Number(m[1])); const number=numbers[0]||roomNumber(name); const images=Array.from(new Set((numbers.length?numbers:[number]).flatMap((n)=>ROOM_GALLERIES[n]||[])));
...
export function AiRoomDetailsEnhancer(){
  ...
  const image=useMemo(()=>room?.images[photo]||room?.images[0]||"",[room,photo]); if(!mounted||!room)return null; const t=COPY[room.language]; const detailNumbers=Array.from(room.name.matchAll(/(?:Room|Δωμάτιο|Zimmer|Chambre|Camera|Habitación|Oda|Apartment|Appartement|Appartamento|Apartamento|Daire|Διαμέρισμα)\s*(10|[1-9])/gi)).map((m)=>Number(m[1])); const featureRooms=Array.from(new Set(detailNumbers.length?detailNumbers:[room.roomNumber])); const features=featureRooms.flatMap((n)=>featureList(n,room.language)).filter((item,index,list)=>list.findIndex((other)=>other.text===item.text)===index);
  return createPortal(<div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 sm:items-center sm:p-5" onMouseDown={(e)=>{if(e.currentTarget===e.target)setRoom(null);}} role="dialog" aria-modal="true" aria-label={room.name}>
    <section className="flex max-h-[calc(100dvh-0.5rem)] w-full max-w-2xl flex-col overflow-x-hidden overflow-y-auto rounded-t-[28px] bg-white shadow-2xl sm:max-h-[90dvh] sm:max-w-xl sm:rounded-[28px]" style={{overscrollBehavior:"contain"}} onWheel={(e)=>e.stopPropagation()} onTouchMove={(e)=>e.stopPropagation()}>
      <div className="relative shrink-0 bg-stone-950"><div data-ai-detail-hero="intrinsic" className="relative w-full shrink-0 overflow-hidden bg-stone-100"><img data-ai-detail-hero-image="true" src={image} alt={`${room.name} ${t.photo.toLowerCase()} ${photo+1}`} className="block h-auto w-full" draggable={false}/></div>
      <button type="button" onClick={()=>setRoom(null)} className="absolute right-3 top-3 z-20 flex h-11 w-11 items-center justify-center rounded-full bg-white text-2xl shadow-lg" aria-label={t.close}>×</button>
      {room.images.length>1?<><button type="button" onClick={()=>setPhoto(v=>(v-1+room.images.length)%room.images.length)} className="absolute left-3 top-1/2 z-20 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/95 text-2xl shadow-lg" aria-label={t.previous}>‹</button><button type="button" onClick={()=>setPhoto(v=>(v+1)%room.images.length)} className="absolute right-3 top-1/2 z-20 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/95 text-2xl shadow-lg" aria-label={t.next}>›</button></>:null}
      <div className="absolute right-3 z-20 rounded-full bg-white/95 px-3 py-1.5 text-xs font-semibold shadow" style={{bottom:"10px"}}>{photo+1}/{room.images.length}</div>
      </div>
      <div data-ai-detail-content="natural" className="flex min-h-0 flex-col p-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] sm:p-4">
        <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-black leading-tight text-stone-950 sm:text-2xl">{room.name}</h2>{room.saving?<span data-ai-detail-saving="prominent" className="inline-flex shrink-0 items-center whitespace-nowrap rounded-full border border-[#c8d99c] bg-[#eef5dc] px-2.5 py-1 text-xs font-black text-[#43551b] shadow-sm">✓ {t.saving}: {room.saving}</span>:null}</div><p className="mt-0.5 text-xs text-stone-500 sm:text-sm">{room.category}</p></div><div className="shrink-0 text-right">{room.originalPrice?<p className="text-[11px] text-stone-400 line-through">{room.originalPrice}</p>:null}<p className="text-xl font-black text-[#43551b] sm:text-2xl">{room.directPrice}</p></div></div>
        ...
        <div data-ai-detail-action="inline" className="pt-3">
          <button type="button" onClick={()=>{room.selectButton?.click();setRoom(null);}} className="w-full rounded-xl bg-[#ff385c] px-5 py-2.5 text-base font-bold text-white shadow-sm">{t.select}</button>
        </div>
