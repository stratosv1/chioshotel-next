...
export function AiRoomDetailsEnhancer(){
  ...
  useEffect(()=>{ const handle=(event:MouseEvent)=>{ const button=(event.target as HTMLElement|null)?.closest<HTMLButtonElement>("button"); const language=DETAILS_LANGUAGE[(button?.textContent||"").trim()]; if(!button||!language)return; const details=readRoomCard(button,language); if(!details)return; event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();setPhoto(0);setExpanded(false);setRoom(details);}; document.addEventListener("click",handle,true); return()=>document.removeEventListener("click",handle,true);},[]);
  ...
  return createPortal(<div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 sm:items-center sm:p-5" onMouseDown={(e)=>{if(e.currentTarget===e.target)setRoom(null);}} role="dialog" aria-modal="true" aria-label={room.name}>
    <section className="flex h-[94dvh] max-h-[820px] w-full max-w-2xl flex-col overflow-hidden rounded-t-[28px] bg-white shadow-2xl sm:rounded-[28px]" style={{overscrollBehavior:"contain"}} onWheel={(e)=>e.stopPropagation()} onTouchMove={(e)=>e.stopPropagation()}>
      <div className="relative shrink-0 bg-stone-950"><div className="relative w-full overflow-hidden bg-stone-950" style={{height:"min(42dvh,360px)"}}><img src={image} alt={`${room.name} ${t.photo.toLowerCase()} ${photo+1}`} className="absolute inset-0 h-full w-full object-contain" draggable={false}/></div>
      ...
      <div className="flex min-h-0 flex-1 flex-col p-4 sm:p-5">
        ...
        <div className={`mt-2 flex flex-wrap content-start gap-2 pr-1 transition-all ${expanded?"max-h-[220px] overflow-y-auto":"max-h-[128px] overflow-hidden"}`}>
          {features.map(({key,text})=><span key={`${key}-${text}`} className="inline-flex items-center gap-1.5 rounded-full border border-stone-200 bg-[#f7f1e8] px-3 py-2 text-[12px] font-semibold text-stone-700"><span aria-hidden>{ICON[key]}</span>{text}</span>)}
        </div>
        {features.length>7?<button type="button" onClick={()=>setExpanded(v=>!v)} className="mt-2 self-start text-sm font-bold text-[#435f12] underline underline-offset-4">{expanded?t.less:t.more}</button>:null}
        <div className="mt-auto pt-3">
          {room.saving?<div className="flex items-center justify-between rounded-2xl bg-[#f3f6e8] px-4 py-3 text-sm text-[#63752d]"><span>{t.saving}</span><strong>{room.saving}</strong></div>:null}
          <button type="button" onClick={()=>{room.selectButton?.click();setRoom(null);}} className="mt-3 w-full rounded-2xl bg-[#ff385c] px-5 py-3.5 text-base font-bold text-white shadow-sm">{t.select}</button>
        </div>
      </div>
