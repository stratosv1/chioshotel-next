...
export function ConversationalRoomSales() {
  ...
  function selectOffer(offer: Offer) {
    const nextChoices = [...choices, { group: activeGroup + 1, guests: guestGroups[activeGroup], offer }];
    ...
    if (nextGroup < roomCount) {
      const remaining = (offersByGroup[nextGroup] || []).filter((candidate) => !new Set(nextChoices.map((choice) => `${choice.offer.roomId}:${choice.offer.unitId}`)).has(`${candidate.roomId}:${candidate.unitId}`));
      if (!remaining.length) {
        addAssistant(t.noAvailability(nextGroup + 1));
        setStep("complete");
        return;
      }
      setActiveGroup(nextGroup);
      addAssistant(t.choose(nextGroup + 1, guestGroups[nextGroup]));
    } else {
      setStep("breakfast");
