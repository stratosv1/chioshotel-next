...
export function ChatMessage({ message }:{ message:ChatItem }) {
  const icon = message.kind === "date" ? "📅 " : message.kind === "room" ? "🛏️ " : message.kind === "guest" ? "👤 " : "";
  return <div className={`msg flex items-end gap-2 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
    ...
    <div className={`relative max-w-[84%] ${message.role === "user" ? "pb-2" : ""}`}>
      <div className={`whitespace-pre-line px-4 py-3 text-[15px] leading-6 shadow-sm ${message.role === "user" ? "rounded-[20px] rounded-br-[6px] bg-[#6b604f] text-white" : "rounded-[20px] rounded-bl-[6px] border border-[#dfd6ca] bg-white"}`}>{message.role === "user" ? icon : ""}{message.content}</div>
