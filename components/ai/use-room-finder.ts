...
class AvailabilityError extends Error{constructor(readonly code:string){super(code);this.name="AvailabilityError";}}
...
export function useRoomFinder(language:RoomFinderLanguage){
 ...
 async function interpret(value:string,current:FinderStep):Promise<Command>{const recentMessages=messages.slice(-8).map(({role,content})=>({role,content}));const controller=new AbortController();const timeout=window.setTimeout(()=>controller.abort(),7_500);try{const response=await fetch("/api/ai-assistant/interpret",{method:"POST",headers:{"Content-Type":"application/json"},signal:controller.signal,body:JSON.stringify({message:value,context:{language,currentStep:current,checkin:checkin||undefined,checkout:checkout||undefined,roomCount:roomCount||undefined,guestGroups:groups,currentRoom:current==="guests"?groups.length+1:undefined,recentMessages}})});const data=await response.json();if(!response.ok||!data?.command)throw new Error();return data.command;}finally{window.clearTimeout(timeout);}}
 ...
 async function searchRooms(label:string){if(!await beginUserTurn(label,"normal","❤️"))return;setStep("searching");setFeedback("idle");add("assistant",tone.searching);setTyping(true);try{const result=await Promise.all(groups.map(async guests=>{const q=new URLSearchParams({checkin,checkout,guests:String(guests),lang:language});const r=await fetch(`/api/ai-room-finder/availability?${q}`,{cache:"no-store"});const p=await r.json();if(!r.ok||!p?.success)throw new AvailabilityError(String(p?.code||"REQUEST_FAILED"));return Array.isArray(p.offers)?p.offers:[];}));setOffers(result);setActiveGroup(0);if(!(result[0]||[]).length){setStep("unavailable");add("assistant",tone.unavailable);}else{setStep("selecting");add("assistant",tone.results(1,groups[0]));}}catch(error){setStep("unavailable");const stale=error instanceof AvailabilityError&&(error.code==="STALE_DATA"||error.code==="DATA_UNAVAILABLE");add("assistant",stale?INVENTORY_UNAVAILABLE[language]:tone.unavailable);}finally{setTyping(false);endUserTurn();}}
