...
export function useRoomFinder(language:RoomFinderLanguage){
 ...
 async function selectOffer(offer:RoomOffer){if(!await beginUserTurn(`${copy.select}: ${offer.name}`,"room","❤️","quick"))return;const next=[...choices,{group:activeGroup+1,guests:groups[activeGroup],offer}];setChoices(next);add("assistant",tone.selected(offer.name));if(roomCount&&activeGroup+1<roomCount){const group=activeGroup+1;setActiveGroup(group);setFeedback("idle");add("assistant",tone.results(group+1,groups[group]));}else{setStep("breakfast");setFeedback("idle");}endUserTurn();}
