    ...
    <main className="flex h-[var(--ai-chat-height,100dvh)] flex-col overflow-hidden bg-[#f6f2eb] text-[#29251f]">
      ...
      <div ref={feedRef} data-ai-chat-scroll="true" className="ai-scroll min-h-0 flex-1 overflow-y-auto overscroll-contain scroll-smooth">
        <div data-ai-conversation-feed="true" className="mx-auto flex min-h-full max-w-3xl flex-col px-3 pb-7 pt-5 sm:px-5">
          <div className="space-y-3.5">
            ...
            {step === "selecting" && currentOffer && (
              <section className="ai-message-in ml-0 sm:ml-10">
                ...
                <article className="overflow-hidden rounded-[26px] border border-[#dcd2c5] bg-white shadow-[0_16px_45px_rgba(70,55,35,.10)]">
                  <div className="relative h-52 sm:h-64">
                    <Image src={currentOffer.image} alt={currentOffer.name} fill sizes="(max-width:640px) 100vw, 700px" className="object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/35 via-transparent to-transparent" />
                    {visibleOffers.length > 1 && (
                      <>
                        <button type="button" onClick={() => setCardIndex(index => (index - 1 + visibleOffers.length) % visibleOffers.length)} className="absolute left-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/92 text-2xl shadow-lg backdrop-blur transition active:scale-90">‹</button>
                        <button type="button" onClick={() => setCardIndex(index => (index + 1) % visibleOffers.length)} className="absolute right-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/92 text-2xl shadow-lg backdrop-blur transition active:scale-90">›</button>
                      </>
                    )}
                  </div>
                  <div className="p-4 sm:p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0"><h2 className="truncate text-xl font-bold">{currentOffer.name}</h2><p className="mt-1 text-sm text-[#746b60]">{currentOffer.category}</p></div>
                      <div className="shrink-0 text-right"><p className="text-xs text-[#b05252] line-through">{money(currentOffer.originalTotal, language)}</p><p className="text-xl font-black text-[#5f7448]">{money(currentOffer.directTotal, language)}</p></div>
                    </div>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2">
                      <button type="button" onClick={() => { setDetail(currentOffer); setPhoto(0); }} className="min-h-12 rounded-2xl border border-[#d8cec1] bg-white px-3 text-sm font-bold transition active:scale-[.98]">{copy.details}</button>
                      <button type="button" onClick={() => selectOffer(currentOffer)} className="min-h-12 rounded-2xl bg-[#66714f] px-3 text-sm font-bold text-white shadow-sm transition hover:bg-[#596244] active:scale-[.98]">{copy.select}</button>
                    </div>
                  </div>
                </article>
                {visibleOffers.length > 1 && <div className="mt-3 flex justify-center gap-1.5">{visibleOffers.map((_, index) => <button type="button" aria-label={copy.roomAria(index + 1)} key={index} onClick={() => setCardIndex(index)} className={`h-1.5 rounded-full transition-all ${index === cardIndex ? "w-6 bg-[#66714f]" : "w-1.5 bg-[#cfc5b8]"}`} />)}</div>}
              </section>
            )}
      ...
      <form onSubmit={submit} data-ai-chat-composer="persistent" className="relative z-30 shrink-0 border-t border-[#ddd4c8] bg-[#fbf8f3]/96 px-3 pb-[max(12px,env(safe-area-inset-bottom))] pt-3 backdrop-blur-xl">
        <div className="mx-auto flex max-w-3xl items-end gap-2 rounded-[24px] border border-[#d8cec1] bg-white p-1.5 shadow-[0_8px_30px_rgba(70,55,35,.08)] transition focus-within:border-[#7a8562] focus-within:shadow-[0_8px_32px_rgba(70,55,35,.13)]">
          <input value={input} onChange={event => setInput(event.target.value)} disabled={!inputEnabled} placeholder={inputEnabled ? copy.placeholder : copy.chooseAbove} aria-label={inputEnabled ? copy.placeholder : copy.chooseAbove} className="min-h-11 min-w-0 flex-1 rounded-[18px] bg-transparent px-3 text-[15px] outline-none placeholder:text-[#9a9187] disabled:cursor-default" />
