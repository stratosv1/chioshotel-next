...
export function GuestAssistant() {
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "Πείτε μου ημερομηνίες και άτομα για διαθεσιμότητα ή γράψτε «δείξε μου το δωμάτιο 2» για να δείτε πρώτα φωτογραφίες και χαρακτηριστικά." },
  ]);
    ...
    <main className="min-h-[100dvh] bg-[#fbfaf7] text-stone-950">
      ...
      <section className="mx-auto flex min-h-[calc(100dvh-82px)] max-w-5xl flex-col px-4 sm:px-8">
        <div className="flex-1 space-y-6 py-8 sm:py-12" aria-live="polite">
                    ...
                    <button key={`${offer.roomId}:${offer.unitId}`} type="button" onClick={() => openDetails(offer)} className="group flex w-full overflow-hidden rounded-2xl border border-stone-200 bg-white text-left shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-300 hover:shadow-md focus:outline-none focus:ring-4 focus:ring-emerald-100">
                      ...
                      <div className="flex min-w-0 flex-1 items-center justify-between gap-4 p-3.5 sm:p-5">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2"><h2 className="truncate text-base font-semibold sm:text-lg">{offer.name}</h2><span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-semibold text-emerald-800">{offer.preview ? "Φωτογραφίες" : "-10%"}</span></div>
          ...
          {loading ? <div className="flex justify-start"><div className="flex items-center gap-2 rounded-[1.4rem] rounded-bl-md border border-stone-200 bg-white px-4 py-3 text-sm text-stone-600 shadow-sm"><span className="h-2 w-2 animate-pulse rounded-full bg-emerald-600" />Επεξεργάζομαι το αίτημά σας…</div></div> : null}
          <div ref={endRef} />
        </div>
        ...
        <div className="sticky bottom-0 border-t border-stone-200 bg-[#fbfaf7]/95 py-4 backdrop-blur sm:py-5">
          ...
          <form onSubmit={handleSubmit} className="flex items-center gap-2 rounded-2xl border border-stone-300 bg-white p-2 shadow-lg shadow-stone-200/60 focus-within:border-emerald-500 focus-within:ring-4 focus-within:ring-emerald-100">
            <input ref={inputRef} value={input} onChange={(event) => setInput(event.target.value)} disabled={loading} maxLength={1200} placeholder="Ρωτήστε για δωμάτιο ή γράψτε ημερομηνίες…" className="min-w-0 flex-1 bg-transparent px-3 py-2.5 text-base outline-none placeholder:text-stone-400" />
      ...
      {detailsOffer ? (
        <div className="fixed inset-0 z-40 flex items-end justify-center bg-stone-950/60 backdrop-blur-sm sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-label={`Λεπτομέρειες για ${detailsOffer.name}`}>
          <div className="flex max-h-[96dvh] w-full max-w-3xl flex-col overflow-hidden rounded-t-[2rem] bg-white shadow-2xl sm:max-h-[92dvh] sm:rounded-[2rem]">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-5 pb-32 sm:p-7 sm:pb-28">
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                {!detailsOffer.preview ? <div className="text-right"><p className="text-xs text-stone-400 line-through">{formatEuro(detailsOffer.originalTotal)}</p><p className="text-2xl font-bold text-emerald-800">{formatEuro(detailsOffer.directTotal)}</p><p className="text-xs text-emerald-700">Κερδίζετε {formatEuro(detailsOffer.saving)}</p></div> : null}
              </div>

              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl bg-stone-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-stone-500">Κατηγορία</p><p className="mt-1 text-sm font-medium">{detailsOffer.category}</p></div>
                <div className="rounded-2xl bg-stone-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-stone-500">Χωρητικότητα</p><p className="mt-1 text-sm font-medium">Έως {detailsOffer.maxGuests} άτομα</p></div>
                <div className="rounded-2xl bg-stone-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-stone-500">Πρόσβαση</p><p className="mt-1 text-sm font-medium">{detailsOffer.floor}</p></div>
              </div>

              {!detailsOffer.preview && search.checkin && search.checkout ? <div className="mt-5 rounded-2xl bg-emerald-50 p-4 text-sm text-emerald-950"><strong>{formatDate(search.checkin)} → {formatDate(search.checkout)}</strong><br />{search.guests} {search.guests === 1 ? "άτομο" : "άτομα"} · {detailsOffer.nights} {detailsOffer.nights === 1 ? "νύχτα" : "νύχτες"}</div> : null}

              <details open className="mt-5 rounded-2xl border border-stone-200 bg-white p-4"><summary className="cursor-pointer font-semibold">Χαρακτηριστικά και παροχές</summary><div className="mt-4 flex flex-wrap gap-2">{unique([detailsOffer.floor, ...detailsOffer.features]).map((feature) => <span key={feature} className="rounded-full border border-stone-200 bg-stone-50 px-3 py-1.5 text-xs text-stone-700">{feature}</span>)}</div></details>
            </div>
            ...
            <div className="absolute inset-x-0 bottom-0 border-t border-stone-200 bg-white/95 p-4 backdrop-blur sm:rounded-b-[2rem] sm:px-7">
              {detailsOffer.preview ? <div className="mx-auto flex max-w-3xl items-center gap-3"><div className="min-w-0 flex-1"><p className="text-xs text-stone-500">Δεν χρειάζονται ημερομηνίες για προβολή</p><p className="font-semibold text-stone-900">Δείτε πρώτα το δωμάτιο</p></div><button type="button" onClick={closeDetails} className="rounded-2xl bg-stone-950 px-6 py-3.5 text-sm font-semibold text-white">Επιστροφή στη συνομιλία</button></div> : <div className="mx-auto flex max-w-3xl items-center gap-3"><div className="min-w-0 flex-1"><p className="text-xs text-stone-500">Τιμή απευθείας</p><p className="text-xl font-bold text-emerald-800">{formatEuro(detailsOffer.directTotal)}</p></div><button type="button" onClick={() => openRequest(detailsOffer)} className="rounded-2xl bg-stone-950 px-6 py-3.5 text-sm font-semibold text-white">Αίτημα κράτησης</button></div>}
            </div>
          </div>
        </div>
      ) : null}
