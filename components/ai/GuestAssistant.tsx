    ...
    <main className="min-h-[100dvh] bg-[#fbfaf7] text-stone-950">
      ...
      <section className="mx-auto flex min-h-[calc(100dvh-82px)] max-w-5xl flex-col px-4 sm:px-8">
        <div className="flex-1 space-y-6 py-8 sm:py-12" aria-live="polite">
          ...
          {loading ? <div className="flex justify-start"><div className="flex items-center gap-2 rounded-[1.4rem] rounded-bl-md border border-stone-200 bg-white px-4 py-3 text-sm text-stone-600 shadow-sm"><span className="h-2 w-2 animate-pulse rounded-full bg-emerald-600" />Ψάχνω διαθεσιμότητα…</div></div> : null}
          <div ref={endRef} />
        </div>
          ...
          <div className="flex max-h-[96dvh] w-full max-w-3xl flex-col overflow-hidden rounded-t-[2rem] bg-white shadow-2xl sm:max-h-[92dvh] sm:rounded-[2rem]">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-5 pb-32 sm:p-7 sm:pb-28">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                ...
                <div className="rounded-2xl bg-stone-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-stone-500">Πρόσβαση</p><p className="mt-1 text-sm font-medium">{detailsOffer.floor || "Δείτε τα χαρακτηριστικά"}</p></div>
              </div>

              <div className="mt-5 rounded-2xl bg-emerald-50 p-4 text-sm text-emerald-950"><strong>{formatDate(search.checkin)} → {formatDate(search.checkout)}</strong><br />{search.guests} {search.guests === 1 ? "άτομο" : "άτομα"} · {detailsOffer.nights} {detailsOffer.nights === 1 ? "νύχτα" : "νύχτες"}</div>

              <div className="mt-5 space-y-3">
                <details open className="rounded-2xl border border-stone-200 bg-white p-4"><summary className="cursor-pointer font-semibold">Χαρακτηριστικά και παροχές</summary><div className="mt-4 flex flex-wrap gap-2">{unique([detailsOffer.floor, ...detailsOffer.features]).map((feature) => <span key={feature} className="rounded-full border border-stone-200 bg-stone-50 px-3 py-1.5 text-xs text-stone-700">{feature}</span>)}</div></details>
                <details className="rounded-2xl border border-stone-200 bg-white p-4"><summary className="cursor-pointer font-semibold">Τι περιλαμβάνει η τιμή</summary><p className="mt-3 text-sm leading-6 text-stone-600">Η τιμή αφορά ολόκληρη τη διαμονή για τις επιλεγμένες ημερομηνίες και περιλαμβάνει την έκπτωση 10% απευθείας κράτησης. Η έκπτωση δεν συνδυάζεται με άλλη προσφορά.</p></details>
                <details className="rounded-2xl border border-stone-200 bg-white p-4"><summary className="cursor-pointer font-semibold">Πώς επιβεβαιώνεται η κράτηση</summary><p className="mt-3 text-sm leading-6 text-stone-600">Στέλνετε αίτημα στη reception και επικοινωνούμε μαζί σας για την τελική επιβεβαίωση. Δεν γίνεται αυτόματη χρέωση ή κράτηση.</p></details>
              </div>
            </div>
