...
export function GuestAssistant() {
  ...
  async function submitBookingRequest(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setRequestSuccess(
        `Το αίτημά σας στάλθηκε στη reception${data.requestId ? ` με αριθμό #${data.requestId}` : ""}. Η reception έλαβε επίσης σύνοψη της συνομιλίας και του ενδιαφέροντός σας. Θα επικοινωνήσουμε μαζί σας για επιβεβαίωση.`,
      );
    } catch (requestError) {
        ...
        <div className="overflow-hidden rounded-[2rem] border border-stone-200/80 bg-white shadow-[0_20px_70px_rgba(41,37,36,0.10)]">
          <header className="relative overflow-hidden border-b border-stone-200 bg-stone-950 px-5 py-8 text-white sm:px-8 sm:py-10">
            ...
            <div className="relative max-w-3xl">
              <span className="inline-flex rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-100">AI Guest Assistant</span>
              <h1 className="mt-4 text-3xl font-semibold tracking-tight sm:text-5xl">Βρείτε το δωμάτιο που σας ταιριάζει</h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-stone-300 sm:text-base">Ζωντανός έλεγχος διαθεσιμότητας, φωτογραφίες δωματίων και τιμή απευθείας κράτησης με έκπτωση 10%.</p>
            </div>
          </header>
            ...
            <aside className="space-y-4">
              <div className="rounded-3xl border border-stone-200 bg-stone-50 p-4 sm:p-5">
                <div className="mb-4 flex items-center justify-between">
                  <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700">Αναζήτηση</p><h2 className="mt-1 text-lg font-semibold">Η διαμονή σας</h2></div>
                  <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-800">Live</span>
                </div>
                <div className="space-y-3">
                  <label className="block text-sm font-medium text-stone-700">Άφιξη<input type="date" min={today} value={checkin} onChange={(event) => setCheckin(event.target.value)} className="mt-1.5 w-full rounded-2xl border border-stone-300 bg-white px-4 py-3 outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100" /></label>
                  <label className="block text-sm font-medium text-stone-700">Αναχώρηση<input type="date" min={checkin || today} value={checkout} onChange={(event) => setCheckout(event.target.value)} className="mt-1.5 w-full rounded-2xl border border-stone-300 bg-white px-4 py-3 outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100" /></label>
                  <label className="block text-sm font-medium text-stone-700">Επισκέπτες<select value={guests} onChange={(event) => setGuests(Number(event.target.value))} className="mt-1.5 w-full rounded-2xl border border-stone-300 bg-white px-4 py-3 outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-100">{[1, 2, 3, 4].map((value) => <option key={value} value={value}>{value} {value === 1 ? "επισκέπτης" : "επισκέπτες"}</option>)}</select></label>
                  <button type="button" onClick={checkAvailability} disabled={loading} className="mt-2 flex w-full items-center justify-center rounded-2xl bg-emerald-700 px-5 py-3.5 text-sm font-semibold text-white hover:bg-emerald-600 disabled:opacity-50">{loading ? "Γίνεται έλεγχος…" : "Δείτε διαθεσιμότητα"}</button>
                </div>
                {checkin && checkout ? <div className="mt-4 rounded-2xl bg-white px-4 py-3 text-xs leading-5 text-stone-600"><strong className="text-stone-900">{formatDate(checkin)}</strong> → <strong className="text-stone-900">{formatDate(checkout)}</strong><br />{guests} {guests === 1 ? "επισκέπτης" : "επισκέπτες"}</div> : null}
              </div>
              <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-4 text-sm leading-6 text-emerald-950"><p className="font-semibold">Καλύτερη τιμή απευθείας</p><p className="mt-1 text-emerald-800">Η έκπτωση 10% εφαρμόζεται μία φορά και δεν συνδυάζεται με άλλη προσφορά.</p></div>
            </aside>
