...
const roomMeta: Record<number, { title: string; subtitle: string; image: string; href: string; features: string[] }> = {
  1: { title: "Pokój na piętrze 1", subtitle: "Pokój dwu- / trzyosobowy", image: "/images/rooms/DSC07776-2-e1675109942622.webp", href: "/pl/pokoje-na-chios/standard/", features: ["Piętro", "Prywatny balkon", "Do 4 gości"] },
  2: { title: "Pokój ekonomiczny 2", subtitle: "Ekonomiczny pokój dwuosobowy", image: "/images/rooms/DSC07803-1.webp", href: "/pl/pokoje-na-chios/economy-double/", features: ["Piętro", "Łóżko podwójne", "2 gości"] },
  3: { title: "Pokój na piętrze 3", subtitle: "Pokój dwu- / trzyosobowy", image: "/images/rooms/DSC07867-1.webp", href: "/pl/pokoje-na-chios/standard/", features: ["Piętro", "Aneks kuchenny", "Do 3 gości"] },
  4: { title: "Pokój na piętrze 4", subtitle: "Pokój dwu- / trzyosobowy", image: "/images/rooms/received_1748354861920234.webp", href: "/pl/pokoje-na-chios/standard/", features: ["Piętro", "Aneks kuchenny", "Balkon"] },
  5: { title: "Pokój na parterze 5", subtitle: "Łatwy dostęp", image: "/images/rooms/voulamandis-house-rooms.webp", href: "/pl/pokoje-na-chios/standard/", features: ["Parter", "Bez schodów", "Do 3 gości"] },
  6: { title: "Pokój ekonomiczny 6", subtitle: "Ekonomiczny pokój dwuosobowy", image: "/images/rooms/received_1753964631359257.webp", href: "/pl/pokoje-na-chios/economy-double/", features: ["Parter", "Bez schodów", "2 gości"] },
  7: { title: "Pokój na parterze 7", subtitle: "Pokój dwu- / trzyosobowy", image: "/images/rooms/double-triple-room.jpg", href: "/pl/pokoje-na-chios/standard/", features: ["Parter", "Dostęp do ogrodu", "Do 3 gości"] },
  8: { title: "Apartament rodzinny 8", subtitle: "Niezależny apartament", image: "/images/rooms/chios-apartments-voulamandis.webp", href: "/pl/apartamenty-na-chios/", features: ["Pełna kuchnia", "Salon", "Do 4 gości"] },
  9: { title: "Apartament rodzinny 9", subtitle: "Niezależny apartament", image: "/images/rooms/chios-apartments-voulamandis.webp", href: "/pl/apartamenty-na-chios/", features: ["Pełna kuchnia", "Salon", "Do 4 gości"] },
  10: { title: "Apartament rodzinny 10", subtitle: "Duży niezależny apartament", image: "/images/rooms/DSC07899.webp", href: "/pl/apartamenty-na-chios/", features: ["Pełna kuchnia", "Układ rodzinny", "Do 5 gości po uzgodnieniu"] },
};
...
export function PolishChiosHotelsLiveSearch() {
  ...
  async function runSearch(event: React.FormEvent) {
    ...
    setError("");
    setPayload(null);
    if (!checkin || !checkout || checkout <= checkin) { setError("Wybierz datę wyjazdu późniejszą niż data przyjazdu."); return; }
    setLoading(true);
    try {
      ...
      setError(searchError instanceof Error ? searchError.message : "Aktualna dostępność jest chwilowo niedostępna.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <section id="live-availability" className="scroll-mt-24 bg-[#263127] px-4 py-12 text-white sm:px-6 sm:py-16 lg:px-8 lg:py-24" aria-labelledby="live-availability-title-pl">
      <div className="mx-auto max-w-7xl">
        ...
        {payload ? <div className="mt-10 sm:mt-12"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-200">WYNIKI NA ŻYWO</p><h3 className="mt-2 text-2xl font-black text-white sm:text-3xl">{availableRooms.length === 1 ? "1 dostępna opcja" : availableRooms.length > 1 ? `${availableRooms.length} dostępnych opcji` : "Brak potwierdzonej opcji dla tych dat"}</h3><p className="mt-2 text-sm text-white/70">{formatDate(checkin)} → {formatDate(checkout)} · {guests} {guests === 1 ? "gość" : "gości"} · {payload.nights || 0} {payload.nights === 1 ? "noc" : "nocy"}</p></div><p className="text-xs text-white/55">Sprawdzone na podstawie aktualnych danych obiektu</p></div>
          {availableRooms.length ? <div className="-mx-4 mt-6 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-5 [scrollbar-width:none] sm:-mx-6 sm:px-6 lg:mx-0 lg:grid lg:grid-cols-2 lg:overflow-visible lg:px-0 lg:pb-0 xl:grid-cols-3">{availableRooms.map((room) => { const number = Number(room.roomNumber || 0); const presentation = roomMeta[number] || { title: room.name || `Pokój ${number || ""}`, subtitle: room.category || "Voulamandis House", image: "/images/rooms/double-triple-room.jpg", href: "/pl/pokoje-na-chios/", features: [room.floor || "Kambos", `${room.maxGuests || guests} gości`] }; const total = Number(room.totalPrice || 0); const nights = Number(room.nights || payload.nights || 1); return <article key={`${room.roomId}:${room.unitId}`} className="min-w-[86vw] snap-start overflow-hidden rounded-[28px] bg-white text-[#2f261f] shadow-2xl sm:min-w-[68vw] lg:min-w-0"><div className="relative h-52"><Image src={presentation.image} alt={`${presentation.title} · Voulamandis House`} fill sizes="(min-width: 1280px) 30vw, (min-width: 1024px) 44vw, 86vw" className="object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-transparent" /><span className="absolute bottom-4 left-4 rounded-full bg-emerald-100 px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.08em] text-emerald-900">Dostępne w tych terminach</span></div><div className="p-5 sm:p-6"><p className="text-[10px] font-black uppercase tracking-[0.15em] text-amber-800">{presentation.subtitle}</p><h4 className="mt-2 text-xl font-black tracking-[-0.03em]">{presentation.title}</h4><div className="mt-4 flex flex-wrap gap-2">{presentation.features.map((feature) => <span key={feature} className="rounded-full bg-amber-50 px-3 py-1.5 text-[10px] font-black text-amber-900 ring-1 ring-amber-900/10">{feature}</span>)}</div><div className="mt-5 flex items-end justify-between gap-4 border-t border-amber-900/10 pt-5"><div><p className="text-xs font-bold text-[#6f6257]">Cały pobyt</p><p className="mt-1 text-2xl font-black">{formatMoney(total)}</p><p className="mt-1 text-xs text-[#6f6257]">≈ {formatMoney(nights > 0 ? total / nights : total)} / noc</p></div><a href={presentation.href} className="inline-flex min-h-11 items-center justify-center rounded-full bg-[#2f261f] px-4 py-2 text-center text-[11px] font-black uppercase tracking-[0.06em] !text-white transition hover:bg-amber-900">Zobacz pokój</a></div></div></article>; })}</div> : <div className="mt-6 rounded-[28px] border border-white/15 bg-white/8 p-6 sm:p-8"><p className="max-w-3xl text-base leading-7 text-white/80">Dla wybranych dat i liczby gości nie pokazuje się teraz potwierdzona opcja. Recepcja może sprawdzić alternatywne daty lub możliwość pobytu z jedną zmianą pokoju.</p><div className="mt-5"><a href={`https://wa.me/306944474226?text=${whatsappText}`} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center justify-center rounded-full bg-emerald-100 px-5 py-3 text-sm font-black !text-emerald-950">Zapytaj przez WhatsApp</a></div></div>}
        </div> : null}
      </div>
    </section>
  );
