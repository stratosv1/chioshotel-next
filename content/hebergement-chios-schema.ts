...
function accommodationId(path: string, cardId: string) {
  return schemaId(path, `hebergement-${cardId}`);
}
...
function buildItemList(data: ChiosAccommodationPageData): SchemaObject {
  ...
  return {
    "@type": "ItemList",
    "@id": itemListId(path),
    name: "Chambres et appartements au Voulamandis House",
...
export function buildHebergementChiosSchema(data: ChiosAccommodationPageData) {
  const path = data.seo.canonicalPath;
  ...
  return buildSchemaGraph([
    ...
    buildHotelSchema({
      path,
      description:
        "Voulamandis House est une maison d’hôtes familiale à Kambos, Chios, proposant des chambres économiques, des chambres au rez-de-chaussée ou à l’étage et des appartements familiaux près de Chios-ville et de l’aéroport.",
    }),
    buildWebsiteSchema(),
    buildImageSchema(
      {
        ...
        caption: "Chambres et appartements au Voulamandis House à Kambos, Chios",
      },
      path,
    ),
    ...
    buildBreadcrumbSchema(path, [{ name: "Hébergement à Chios", path }]),
    faq,
  ]);
}
