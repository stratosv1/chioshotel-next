...
function accommodationId(path: string, cardId: string) {
  return schemaId(path, `alloggio-${cardId}`);
}
...
function buildItemList(data: ChiosAccommodationPageData): SchemaObject {
  ...
  return {
    "@type": "ItemList",
    "@id": itemListId(path),
    name: "Camere e appartamenti al Voulamandis House",
...
export function buildAlloggioChiosSchema(data: ChiosAccommodationPageData) {
  const path = data.seo.canonicalPath;
  ...
  return buildSchemaGraph([
    ...
    buildHotelSchema({
      path,
      description:
        "Voulamandis House è una struttura ricettiva a gestione familiare a Kambos, Chios, con camere economy, camere al piano terra e superiore e appartamenti familiari vicino alla città e all’aeroporto.",
    }),
    buildWebsiteSchema(),
    buildImageSchema(
      {
        ...
        caption: "Camere e appartamenti al Voulamandis House a Kambos, Chios",
      },
      path,
    ),
    ...
    buildBreadcrumbSchema(path, [{ name: "Alloggio a Chios", path }]),
    faq,
  ]);
}
