...
Για ΚΑΘΕ υποκεφάλαιο που σου παρέχεται, δημιούργησε ΑΚΡΙΒΩΣ ΕΝΑ κεντρικό, ολοκληρωμένο διαδραστικό Lab για τη βασική φυσική έννοια που διδάσκεται.

Δεν επιτρέπεται να παραλείψεις ολόκληρο υποκεφάλαιο επειδή κάποιες προχωρημένες εφαρμογές του απαιτούν διαφορετικό renderer. Σε αυτή την περίπτωση δημιούργησε κανονικά το ένα βασικό Lab για την κύρια έννοια με το διαθέσιμο ασφαλές physicsPreset και βάλε μόνο τις μη αποδόσιμες ειδικές επεκτάσεις στο nonInteractiveCore.
...
Για γνωστές βασικές έννοιες χρησιμοποίησε το αντίστοιχο πραγματικό renderer:
- Οριζόντια βολή → horizontal_projectile
- Ομαλή κυκλική κίνηση → uniform_circular_motion
- Κεντρομόλος δύναμη → centripetal_force
...
Αν το διαθέσιμο renderer δεν μπορεί να αποδώσει μια ειδική επέκταση της έννοιας σωστά, μην εφευρίσκεις ψεύτικο διάγραμμα. Δημιούργησε παρ' όλα αυτά το βασικό Lab του υποκεφαλαίου όταν η κύρια έννοια υποστηρίζεται από διαθέσιμο physicsPreset, και κατάγραψε μόνο το μη υποστηριζόμενο ειδικό core material στο nonInteractiveCore.
...
ΥΠΟΧΡΕΩΤΙΚΑ:
- επέστρεψε μία section για ΚΑΘΕ παρεχόμενο υποκεφάλαιο,
- κάθε section πρέπει να έχει ΑΚΡΙΒΩΣ 1 widget,
- μην επιστρέψεις δεύτερο widget για το ίδιο υποκεφάλαιο,
- μην παραλείψεις την Ομαλή κυκλική κίνηση όταν υπάρχει στο παρεχόμενο υλικό· χρησιμοποίησε uniform_circular_motion.
