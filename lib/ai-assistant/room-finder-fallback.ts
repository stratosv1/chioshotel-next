...
const NIGHT_WORDS = /(νύχτ(?:α|ες)|νυχτ(?:α|ες)|βραδι(?:ά|ες)|διανυκτερ(?:εύσεις|ευσεις|εύση|ευση)|nights?|nächte|nachte|nuits?|notti|noches|gece)/iu;
...
const NIGHT_WORD_SOURCE = "(?:νυχτ(?:α|ες)|βραδι(?:α|ες)|διανυκτερ(?:ευσεις|ευση)|nights?|nachte|nuits?|notti|noches|gece)";
...
const MONTH_PATTERN = [...MONTH_LOOKUP.keys()].sort((a, b) => b.length - a.length).map(escapeRegExp).join("|");
...
function pushUniqueDate(values: Array<{ value: string; index: number }>, seen: Set<string>, day: number, month: number, rawYear: string | undefined, today: string, index: number) {
  const value = isoFromParts(day, month, normalizeYear(rawYear, day, month, today));
...
function extractDates(text: string, today: string) {
  ...
  for (const match of normalized.matchAll(new RegExp(NUMERIC_DATE_SOURCE, "gu"))) {
    pushUniqueDate(values, seen, Number(match[1]), Number(match[2]), match[3], today, match.index ?? 0);
  }

  const condensedRange = new RegExp(`${TOKEN_START}(\\d{1,2})\\s*${RANGE_SEPARATOR}\\s*(\\d{1,2})\\s+(?:του\\s+)?(${MONTH_PATTERN})(?:\\s+(\\d{2,4}))?${TOKEN_END}`, "giu");
  for (const match of normalized.matchAll(condensedRange)) {
    const month = MONTH_LOOKUP.get(match[3]) || 0;
    const index = match.index ?? 0;
    const year = normalizeYear(match[4], Number(match[1]), month, today);
    const first = isoFromParts(Number(match[1]), month, year);
    const second = isoFromParts(Number(match[2]), month, year);
    if (first && !seen.has(first)) { seen.add(first); values.push({ value: first, index }); }
    if (second && !seen.has(second)) { seen.add(second); values.push({ value: second, index: index + 1 }); }
  }

  const monthFirstRange = new RegExp(`${TOKEN_START}(${MONTH_PATTERN})\\s+(\\d{1,2})\\s*${RANGE_SEPARATOR}\\s*(\\d{1,2})(?:,?\\s+(\\d{2,4}))?${TOKEN_END}`, "giu");
  ...
  const namedDayFirst = new RegExp(`${TOKEN_START}(\\d{1,2})\\.?\\s+(?:του\\s+)?(${MONTH_PATTERN})(?:\\s+(\\d{2,4}))?${TOKEN_END}`, "giu");
  for (const match of normalized.matchAll(namedDayFirst)) {
    pushUniqueDate(values, seen, Number(match[1]), MONTH_LOOKUP.get(match[2]) || 0, match[3], today, match.index ?? 0);
  }

  const namedMonthFirst = new RegExp(`${TOKEN_START}(${MONTH_PATTERN})\\s+(\\d{1,2})(?:,?\\s+(\\d{2,4}))?${TOKEN_END}`, "giu");
  for (const match of normalized.matchAll(namedMonthFirst)) {
    pushUniqueDate(values, seen, Number(match[2]), MONTH_LOOKUP.get(match[1]) || 0, match[3], today, match.index ?? 0);
  }
...
function numberBeforeWords(text: string, words: RegExp, normalizedWordSource: string, max: number) {
  ...
  for (const [value, aliases] of NUMBER_WORDS) {
    ...
    for (const alias of aliases) {
      if (new RegExp(`${TOKEN_START}${escapeRegExp(normalizeSearchText(alias))}\\s*(?:${normalizedWordSource})${TOKEN_END}`, "iu").test(normalized)) return value;
    }
  }
  return null;
}
...
function matchedPreferences(text: string) {
  return PREFERENCE_PATTERNS.filter(([, pattern]) => pattern.test(text)).map(([preference]) => preference);
}

function preferenceUpdateFromText(text: string, current: readonly RoomFinderPreference[]) {
  const matches = matchedPreferences(text);
  const removal = PREFERENCE_REMOVAL_WORDS.test(text);
  if (GENERIC_PREFERENCE_CLEAR.test(text) && matches.length === 0) return { recognized: true, preferences: [] as RoomFinderPreference[] };
  if (removal) {
    if (matches.length === 1) return { recognized: true, preferences: current.filter(preference => preference !== matches[0]) };
    return { recognized: false, preferences: [...current] };
  }
  if (PREFERENCE_QUESTION.test(text) && !PREFERENCE_INTENT.test(text)) return { recognized: false, preferences: [...current] };
...
export function hasCoreBookingActions(command: RoomFinderCommand | null | undefined) {
  return Boolean(command?.actions.some(action => action.type === "set_stay_dates" || action.type === "set_room_count" || action.type === "set_guest_count" || action.type === "restart_search"));
}
...
export function canUseDeterministicCommandDirectly(message: string, context: RoomFinderConversationContext, command: RoomFinderCommand | null | undefined) {
  if (!command?.actions.length) return false;
  if (command.actions.some(action => action.type === "restart_search")) return true;
  const hasDateAction = command.actions.some(action => action.type === "set_stay_dates" && Boolean(action.checkin || action.checkout || action.nights));
  ...
  const mentionsDate = NUMERIC_DATE_HINT.test(normalizeSearchText(message)) || hasNamedMonth(message) || CHECKIN_WORDS.test(message) || CHECKOUT_WORDS.test(message);
  ...
  if (context.currentStep === "checkin" || context.currentStep === "checkout") return hasDateAction;
  if (context.currentStep === "rooms") return hasRoomAction;
  if (context.currentStep === "guests") return hasGuestAction;
  return false;
}

export function fallbackRoomFinderCommand(message: string, context: RoomFinderConversationContext = {}): RoomFinderCommand | null {
  const text = message.trim();
  if (!text) return null;
  const language = context.language || "en";
  if (RESTART_WORDS.test(text)) return { language, replyMode: "execute", actions: [{ type: "restart_search" }] };
  ...
  if (dates.length >= 2 && dates[1].value > dates[0].value) {
    ...
    if (CHECKOUT_WORDS.test(text) || context.currentStep === "checkout") actions.push({ type: "set_stay_dates", checkout: value });
    else if (CHECKIN_WORDS.test(text) || context.currentStep === "checkin") actions.push({ type: "set_stay_dates", checkin: value });
  }
  ...
  if (nights && !hasExplicitCheckout) actions.push({ type: "set_stay_dates", nights });

  const roomCount = numberBeforeWords(text, ROOM_WORDS, ROOM_WORD_SOURCE, 99);
  if (roomCount) actions.push({ type: "set_room_count", roomCount });
  ...
  if (explicitGuests) {
    if (context.currentStep === "guests" && context.currentRoom && explicitGuests <= 5) actions.push({ type: "set_guest_count", guestRoom: context.currentRoom, guests: explicitGuests });
    else actions.push({ type: "set_guest_count", totalGuests: explicitGuests });
    ...
    if (adults != null && children != null && adults + children >= 1 && adults + children <= 15) actions.push({ type: "set_guest_count", totalGuests: adults + children });
  } else if (context.currentStep === "guests" && /^\s*[1-5]\s*$/.test(text)) {
    const guests = Number(text);
    actions.push(context.currentRoom ? { type: "set_guest_count", guestRoom: context.currentRoom, guests } : { type: "set_guest_count", totalGuests: guests });
  }

  const preferenceUpdate = preferenceUpdateFromText(text, context.preferences || []);
  if (preferenceUpdate.recognized) actions.push({ type: "set_preferences", preferences: preferenceUpdate.preferences });
