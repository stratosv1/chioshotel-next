...
function diagnoseDecline(row: any) {
  ...
  const previousCtr = previousImpressions > 0 ? num(row.previous_clicks) / previousImpressions : 0;
  ...
  const positionDelta = previousPosition > 0 && currentPosition > 0 ? currentPosition - previousPosition : 0;
...
async function getPageQueryChanges(
  ...
  return (rows as any[]).map((row) => {
    ...
    return {
      ...
      positionDelta: previousPosition > 0 && currentPosition > 0 ? currentPosition - previousPosition : 0,
    };
  });
}
...
export async function getSeoAdvisorData(siteUrl = DEFAULT_SITE) {
  ...
  if (declining) {
    ...
    const queryBreakdown = await getPageQueryChanges(sql, siteUrl, latestDate, page);
    priorities.push({
      severity: "high",
      title: `Έπεσε αισθητά η σελίδα ${shortPath(page)}`,
      explanation: "Η σελίδα έχασε οργανικά clicks σε σχέση με τις προηγούμενες 28 ημέρες. Η πρόταση δεν βασίζεται μόνο στο ποσοστό πτώσης: συγκρίνουμε θέση, εμφανίσεις, CTR και πλέον τις queries που άλλαξαν περισσότερο.",
      diagnosis: diagnoseDecline(declining),
      action: "Πρώτα εξετάζουμε τις queries που έχασαν clicks ή θέσεις. Μόνο μετά αποφασίζουμε αν χρειάζεται αλλαγή σε title/meta, κύριο περιεχόμενο ή internal links. Δεν αλλάζουμε URL ή canonical χωρίς σαφή τεχνικό λόγο.",
  ...
  if (lowCtr) {
    ...
    priorities.push({
      ...
      explanation: "Η Google εμφανίζει συχνά τη σελίδα και η μέση θέση είναι ήδη στην πρώτη σελίδα, αλλά σχετικά λίγοι χρήστες κάνουν click. Για να μην αλλάξουμε λάθος snippet, εξετάζουμε ποιες queries έχουν πραγματικά υψηλές εμφανίσεις και χαμηλό CTR.",
      diagnosis: "Το πρώτο πράγμα που πρέπει να ξεχωρίσουμε είναι αν το χαμηλό CTR αφορά τις βασικές εμπορικές/θεματικές queries ή αν προέρχεται από πιο γενικές αναζητήσεις όπου ένα χαμηλότερο CTR είναι φυσιολογικό.",
      action: "Βελτιώνουμε title και meta description μόνο για την πρόθεση των queries που έχουν αρκετές εμφανίσεις και καλή θέση. Μετά ελέγχουμε αν το κύριο περιεχόμενο απαντά ακριβώς σε αυτές τις αναζητήσεις.",
  ...
  if (opportunity) {
    priorities.push({
      ...
      diagnosis: "Εδώ δεν έχουμε απαραίτητα πρόβλημα. Έχουμε query που ήδη αποδίδει αρκετά ώστε μια μικρή βελτίωση θέσης να μπορεί να φέρει περισσότερα clicks.",
  ...
  if (ctrChange <= -20 && current.impressions >= 100) {
    priorities.push({
      ...
      diagnosis: "Το συνολικό CTR είναι ένδειξη, όχι από μόνο του διάγνωση. Για απόφαση κοιτάμε πρώτα τις επιμέρους σελίδες και queries με αρκετό όγκο.",
  ...
  if (priorities.length === 0) {
    priorities.push({
      ...
      diagnosis: "Η απουσία επείγοντος προβλήματος είναι θετικό εύρημα. Συνεχίζουμε με ευκαιρίες ανάπτυξης αντί για διορθώσεις χωρίς αποδείξεις.",
