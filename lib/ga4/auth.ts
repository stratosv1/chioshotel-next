...
async function exchangeAnalyticsAccessToken(request: Request): Promise<string> {
  ...
  const iamResponse = await fetch(impersonationUrl, {
    ...
    body: JSON.stringify({ scope: [ANALYTICS_SCOPE], lifetime: "3600s" }),
    cache: "no-store",
  });
...
/**
 * Uses the same keyless Vercel OIDC -> Workload Identity -> service account
 * chain as Search Console, but requests only the Analytics read-only scope.
 */
export function getAnalyticsAccessToken(request: Request): Promise<string> {
  const cached = accessTokenByRequest.get(request);
  if (cached) return cached;
  const pending = exchangeAnalyticsAccessToken(request);
