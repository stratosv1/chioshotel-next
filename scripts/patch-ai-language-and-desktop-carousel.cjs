...
const isAlreadyPatched =
  source.includes('const [languageChosen, setLanguageChosen]') &&
  source.includes('function chooseLanguage') &&
  source.includes('function scrollOffers') &&
  source.includes('Choose your language') &&
  source.includes('aria-label="Previous rooms"') &&
  source.includes('aria-label="Next rooms"') &&
  source.includes('t.topPick') &&
  source.includes('t.directDeal') &&
  source.includes('t.noCard');

if (isAlreadyPatched) {
  console.log('AI language chooser and room carousel already applied; skipping');
  process.exit(0);
}
