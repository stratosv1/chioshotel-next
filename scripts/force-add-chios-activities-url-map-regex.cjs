...
if (!markerRegex.test(text)) {
  console.error("ERROR: Still could not find /mike-2/ route with regex.");
  process.exit(1);
}

text = text.replace(markerRegex, block + "$1");

fs.writeFileSync(file, text, "utf8");

console.log("SUCCESS: Added 42 Chios Activities localized routes to lib/url-map.ts.");
