...
const newTitleRow = '<div className="flex items-start justify-between gap-4"><div className="min-w-0"><h2 className="text-2xl font-black text-stone-950">{room.name}</h2><p className="mt-1 text-sm text-stone-500">{room.category}</p></div><div className="shrink-0 text-right">{room.originalPrice?<p className="text-xs text-stone-400 line-through">{room.originalPrice}</p>:null}<p className="text-2xl font-black text-[#43551b]">{room.directPrice}</p></div></div>{room.images.length>1?<div className="mt-3 flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label={`${t.photo} thumbnails`}>{room.images.map((src,i)=><button key={src} type="button" onClick={()=>setPhoto(i)} className={`relative h-12 w-16 shrink-0 overflow-hidden rounded-lg border-2 bg-white shadow-sm ${i===photo?"border-[#ff385c] ring-1 ring-[#ffccd6]":"border-stone-200 opacity-80"}`} aria-label={`${t.photo} ${i+1}`}><img src={src} alt="" className="h-full w-full object-cover" loading="lazy" draggable={false}/></button>)}</div>:null}';
...
if (!source.includes('mt-3 flex gap-2 overflow-x-auto pb-1')) throw new Error("White-area thumbnail row was not applied");

fs.writeFileSync(file, source);
console.log("Moved AI room detail thumbnails below title");
