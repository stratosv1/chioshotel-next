...
const thumbnailRowClass = 'mt-3 mb-3 flex min-h-[64px] shrink-0 items-start gap-2.5 overflow-x-auto pb-1.5 pt-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden';
...
if (!source.includes('data-ai-detail-thumbnails="white"')) {
  ...
  source = source.replace(
    /className="mt-3[^\"]*\[&::\-webkit\-scrollbar\]:hidden"(?= aria-label=\{`\$\{t\.photo\} thumbnails`\})/,
    `className="${thumbnailRowClass}"`,
  );
  source = source
...
if (!source.includes('min-h-[64px] shrink-0 items-start')) throw new Error("Thumbnail row spacing was not applied");
if (!source.includes('h-14 w-20')) throw new Error("Large thumbnail sizing was not applied");

fs.writeFileSync(file, source);
console.log("AI room detail thumbnails given dedicated space below title");
