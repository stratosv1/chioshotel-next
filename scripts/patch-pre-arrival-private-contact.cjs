...
const enPageFile = "app/pre-arrival/page.tsx";
const localePageFile = "app/[locale]/pre-arrival/page.tsx";

backup(contentFile, "private-contact-i18n");
backup(componentFile, "private-contact-i18n");
backup(layoutFile, "hide-header-pre-arrival");
backup(enPageFile, "noindex");
backup(localePageFile, "noindex");

/* -------------------------------------------------------
   1. Rewrite content/pre-arrival.ts with all languages
------------------------------------------------------- */
const content = `
...
fs.writeFileSync(contentFile, content.replace(/^\n/, ""), "utf8");

/* -------------------------------------------------------
   2. Patch component for surname + SMS + email + proper contact
------------------------------------------------------- */
...
`return { guest: "", surname: "", room: "", checkin: "", checkout: "", price: "" };`
);
...
if (!component.includes("const communicationLinks = useMemo")) {
  component = component.replace(
`  const stayText = useMemo(() => {`,
`  const communicationLinks = useMemo(() => {
    const surname = booking.surname || booking.guest || "";
    const smsMessage = [
      "Voulamandis House arrival message",
    ...
    const whatsappMessage = [
      "Hello Voulamandis House, I would like to send my arrival time.",
      surname ? \`Surname: \${surname}\` : "",
      booking.checkin ? \`Check-in: \${booking.checkin}\` : "",
    ]
      .filter(Boolean)
    ...
    const subject = encodeURIComponent("Voulamandis House pre-arrival");
    const emailBody = encodeURIComponent(smsMessage);

    return {
      whatsapp: \`\${preArrivalContact.whatsappBase}?text=\${encodeURIComponent(whatsappMessage)}\`,
      sms: \`\${preArrivalContact.smsBase}?body=\${encodeURIComponent(smsMessage)}\`,
      email: \`mailto:\${preArrivalContact.email}?subject=\${subject}&body=\${emailBody}\`,
    };
  }, [booking.surname, booking.guest, booking.checkin]);
...
`              <InfoCard label={data.booking.guest} value={booking.guest} fallback={data.booking.missingGuest} />
              <InfoCard label={data.booking.surname} value={booking.surname} fallback={data.booking.missingSurname} />
              <InfoCard label={data.booking.room} value={booking.room} fallback={data.booking.missingRoom} />`
);
...
`              <a href={preArrivalContact.phoneHref} className="inline-flex min-h-[54px] items-center justify-center rounded-full bg-white px-6 text-sm font-black uppercase tracking-[0.08em] !text-slate-950">
              ...
              <a href={communicationLinks.whatsapp} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-[54px] items-center justify-center rounded-full border border-white/20 px-6 text-sm font-black uppercase tracking-[0.08em] !text-white">
                {data.help.whatsapp}: {preArrivalContact.whatsappDisplay}
              </a>
              <a href={communicationLinks.email} className="inline-flex min-h-[54px] items-center justify-center rounded-full border border-white/20 px-6 text-sm font-black uppercase tracking-[0.08em] !text-white">
                {data.help.email}: {preArrivalContact.email}
              </a>
              <a href={communicationLinks.sms} className="inline-flex min-h-[54px] items-center justify-center rounded-full border border-white/20 px-6 text-sm font-black uppercase tracking-[0.08em] !text-white">
...
/* -------------------------------------------------------
   3. Hide normal header on pre-arrival pages
------------------------------------------------------- */
let layout = fs.readFileSync(layoutFile, "utf8");

if (!layout.includes("isPreArrivalPage")) {
...
/* -------------------------------------------------------
   4. Add noindex metadata to pre-arrival pages
------------------------------------------------------- */
function addNoIndexMetadata(file) {
  let s = fs.readFileSync(file, "utf8");

  if (!s.includes("robots:")) {
    s = s.replace(
...
`export const metadata: Metadata = {
  ...
  if (s.includes("return buildPageMetadata({") && !s.includes("robots: {")) {
    s = s.replace(
...
addNoIndexMetadata(enPageFile);
addNoIndexMetadata(localePageFile);

/* -------------------------------------------------------
   5. Check sitemap does not contain pre-arrival
------------------------------------------------------- */
const sitemapFile = "app/sitemap.ts";
if (fs.existsSync(sitemapFile)) {
  const sitemap = fs.readFileSync(sitemapFile, "utf8");
  if (sitemap.includes("pre-arrival")) {
    console.warn("WARNING: sitemap.ts contains pre-arrival. Remove it manually.");
  } else {
    console.log("Sitemap OK: no pre-arrival found.");
  }
}
...
const finalLayout = fs.readFileSync(layoutFile, "utf8");

for (const check of [
  ["call phone", "+30 6944764654", finalContent],
  ["whatsapp phone", "+30 6944474226", finalContent],
  ["email", "chioshotel@gmail.com", finalContent],
  ["sms link", "sms:+306944474226", finalContent],
  ["surname query", "guestlastname", finalComponent],
  ["sms body", "Check-in", finalComponent],
  ["hide header", "isPreArrivalPage", finalLayout],
]) {
  console.log(check[0], check[2].includes(check[1]) ? "OK" : "MISSING");
}
