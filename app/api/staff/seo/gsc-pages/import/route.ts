...
function normalizeHeader(value: string) {
  return value.replace(/^\uFEFF/, "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9α-ω]+/g, " ").trim();
}
...
function readIssueFromMetadata(buffer: Buffer | null) {
  if (!buffer) return "";
  for (const row of readCsvRows(buffer)) {
...
function parseOverviewIssues(buffer: Buffer | null, severity: GscPagesOverviewIssue["severity"]) {
  if (!buffer) return [];
  return readCsvRows(buffer).map((row) => ({
...
function parseRows(data: Record<string, unknown>[], fallbackIssue: string) {
  ...
  for (const row of data) {
    ...
    const issue = pickCell(row, ["Issue", "Reason", "Category", "Problem", "Page indexing issue", "Coverage state"]) || fallbackIssue;
    if (!issue) { missingIssueRows += 1; continue; }
    parsed.push({
      url: url.toString(),
      issue,
      lastCrawled: pickCell(row, ["Last crawled", "Last crawl", "Last crawl time", "Last crawled date"]),
      raw: row,
    });
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!isZip && !isCsv) return json({ ok: false, error: "Υποστηρίζεται το ZIP export του Google Search Console ή αρχείο CSV." }, 400);

    const maxBytes = isZip ? MAX_ZIP_BYTES : MAX_CSV_BYTES;
    if (file.size <= 0 || file.size > maxBytes) return json({ ok: false, error: `Το αρχείο πρέπει να είναι έως ${isZip ? "15" : "5"} MB.` }, 400);
    ...
    if (isZip) {
      ...
      if (contents.kind === "overview") {
        ...
        if (!issues.length) return json({ ok: false, error: "Το Pages overview ZIP δεν περιέχει αναγνώσιμες κατηγορίες προβλημάτων." }, 400);
      ...
      if (!tableBuffer) return json({ ok: false, error: "Το drill-down ZIP δεν περιέχει Table.csv." }, 400);
      ...
      if (data.length > MAX_ROWS) return json({ ok: false, error: `Το export έχει πάνω από ${MAX_ROWS.toLocaleString("el-GR")} γραμμές.` }, 400);
      ...
      if (!parsed.length) {
        const detail = missingIssueRows ? " Δεν βρέθηκε κατηγορία Issue στο Metadata.csv. Επίλεξε κατηγορία από το dropdown και ξαναδοκίμασε." : " Χρειάζεται Table.csv με στήλη URL και URLs του chioshotel.gr.";
      ...
      return json({ ok: true, ...result, importFormat: "zip-drilldown", detectedIssue: detectedIssue || null, skippedRows: Math.max(0, data.length - parsed.length) });
    }
    ...
    if (!parsed.length) {
      const detail = missingIssueRows ? " Επίλεξε κατηγορία από το dropdown και ξαναδοκίμασε." : " Χρειάζεται CSV με στήλη URL και URLs του chioshotel.gr.";
      return json({ ok: false, error: `Δεν βρέθηκαν έγκυρα URLs για εισαγωγή.${detail}` }, 400);
    }
    const result = await importGscPagesRows({ fileName: file.name, rows: parsed, originalRowCount: data.length });
