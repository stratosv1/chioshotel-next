...
import { AgentRoomsGuide, type AgentRoom } from "@/components/agents/AgentRoomsGuide";

export const metadata: Metadata = {
  title: "Agent Room Allocation Guide",
  description: "Private room allocation guide for Voulamandis House travel partners.",
  robots: {
    index: false,
    follow: false,
    nocache: true,
...
const getAgentRoomGuideData = unstable_cache(
  async () => {
    if (!process.env.DATABASE_URL) {
      throw new Error("DATABASE_URL is missing for the agent room guide");
    }
    ...
    const amenityMap = new Map(amenityRows.map(row => [row.amenity_key, row.label]));
  ...
  ["agent-room-guide-neon-v1"],
  { revalidate: 3600 },
);

export default async function AgentRoomsPage() {
  const { rooms, commonAmenities } = await getAgentRoomGuideData();

  return <AgentRoomsGuide rooms={rooms} commonAmenities={commonAmenities} />;
}
